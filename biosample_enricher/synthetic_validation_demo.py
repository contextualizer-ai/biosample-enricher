...
def map_synthetic_to_model(synthetic_data: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        'sample_id': synthetic_data.get('nmdc_biosample_id') or synthetic_data.get('gold_biosample_id'),
...
def validate_synthetic_biosamples(input_file: Path) -> dict[str, Any]:
    ...
    results['validation_summary'] = {
        ...
        'enrichable_count': sum(1 for sample in results['valid_samples'] if sample['is_enrichable']),
        'validation_rate': len(results['valid_samples']) / len(biosamples) if biosamples else 0,
        'enrichable_rate': sum(1 for sample in results['valid_samples'] if sample['is_enrichable']) / len(biosamples) if biosamples else 0
    }
...
@click.command()
@click.option("--input-file", "-i", default="data/input/synthetic_biosamples.json", help="Input synthetic biosamples JSON file")
@click.option("--output-file", "-o", help="Output validation results file (default: stdout)")
...
def main(input_file: str, output_file: str, indent: int) -> None:
    ...
    try:
        ...
        click.echo(f"   Valid samples: {summary['valid_count']} ({summary['validation_rate']:.1%})")
        click.echo(f"   Invalid samples: {summary['invalid_count']}")
        click.echo(f"   Enrichable samples: {summary['enrichable_count']} ({summary['enrichable_rate']:.1%})")
        click.echo()
        
        # Print valid samples
        for sample in results['valid_samples']:
            location = sample['location_model']
            click.echo(f"✅ Sample {sample['index']}: Valid ({'enrichable' if sample['is_enrichable'] else 'not enrichable'})")
